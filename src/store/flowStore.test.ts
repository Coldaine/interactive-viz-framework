...
describe('FlowStore', () => {
  ...
  describe('Persistence', () => {
    it('should maintain state across multiple hook instances', () => {
      // First hook instance: add a node
      ...
      // Second hook instance: should see the same state
    ...
    it('should maintain viewport state across instances', () => {
      const { result: result1 } = renderHook(() => useFlowStore())

      act(() => {
        result1.current.setViewport({ x: 100, y: 200, zoom: 1.5 })
      })

      const { result: result2 } = renderHook(() => useFlowStore())

      expect(result2.current.viewport).toEqual({ x: 100, y: 200, zoom: 1.5 })
    })
  })
